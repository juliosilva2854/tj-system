...
Master/Admin do tenant podem ler. Apenas master altera (visao do master).
Admin pode habilitar/desabilitar dentro do seu tenant."""
...
async def update_warehouse_modules(
    ...
    user: dict = Depends(require_roles("master", "admin"))
):
    wh = await db.warehouses.find_one({"id": wid}, {"_id": 0})
    if not wh:
        raise HTTPException(status_code=404, detail="Deposito nao encontrado")
